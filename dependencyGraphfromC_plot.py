...
def save_graph_image(graph, filename):
    """
    Saves a high-resolution, well-spaced image of the dependency graph.
    """
    if not graph.nodes():
        print(f"Error: The generated graph is empty. Check your C syntax.")
        return

    num_nodes = len(graph.nodes())
    
    # Scaling the canvas: 10x10 minimum, grows with node count
    figsize = max(10, int(np.sqrt(num_nodes) * 1.5))
    plt.figure(figsize=(figsize, figsize))

    # Layout: 'k' controls the distance between nodes. 
    # Increase k to push nodes further apart.
    pos = nx.spring_layout(graph, k=2 / np.sqrt(num_nodes), iterations=50)
    ...
    # Draw Edges with slight curves to avoid overlaps
    ...
    plt.title(f"Dependency Graph: {os.path.basename(filename)}", fontsize=16, pad=20)
    plt.axis('off')

    # Save with tight bounding box to remove wasted white space
    print(f"Saving high-res plot to: {filename}")
    plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close()

def main():
    parser = argparse.ArgumentParser(description="Generate a dependency graph from a single C source file.")
    parser.add_argument("file_path", help="Path to the C source file (e.g., ex1.txt or code.c)")
    parser.add_argument("-o", "--output", help="Custom output image name (default: input_filename.png)")
    ...
    # Initialize the required global object
    processVariable._DependencyGraphObj = DependencyGraphfromCFunction()
    ...
    # Process Graph
    analyzer = DependencyGraphfromCFunction()
    dg = analyzer.getDependencyGraph(source_code)

    # Determine output filename
    if args.output:
        output_file = args.output
    else:
        # Defaults to 'filename.png' in the current directory
        base_name = os.path.splitext(os.path.basename(args.file_path))[0]
        output_file = f"{base_name}_graph.png"

    # Export
    save_graph_image(dg, output_file)
    print("Done.")
