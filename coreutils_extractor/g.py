#!/usr/bin/env python3
"""
Preprocess only src/ C files from a Makefile build log, ignoring all warnings.
    ...
    with open(logfile, 'r', errors='ignore') as fh:
        for raw in fh:
            ...
            if re.search(r'\b(gcc|clang|cc)\b', line) and '-c' in line and '.c' in line:
                if "cksum.c" in line:
                    line = line.replace("`test -f 'src/cksum.c' || echo './'`src/cksum.c", "src/cksum.c") # coreutils specific fix
                cmds.append(line)
    return cmds
...
def build_preprocess_cmd(args, cfile, outpath):
    """Transform a compile command into a preprocessing command with -w to suppress warnings."""
    new_args = []
    i = 0
    while i < len(args):
        tok = args[i]
    ...
    # Insert -E -P -w if not already present
...
def main():
    ...
    for line in compile_cmds:
        ...
        if not cfile or not str(cfile).startswith("src/"):
            continue  # silently skip non-src files

        if args.filter and args.filter not in cfile:
            continue

        # Output file: outdir/<basename>.i
        bn = Path(obj).stem
